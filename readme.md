<p align="center">
  <img src="assets/logos/findron.png" alt="ФинДрон" width="160"/>
</p>

<h1 align="center">ФинДрон — IT-агрегатор решений для БАС</h1>

<p align="center">
  Vendor-agnostic платформа управления беспилотными авиационными системами
</p>

<p align="center">
  <img src="https://img.shields.io/badge/версия-1.0-0D7C66?style=flat-square"/>
  <img src="https://img.shields.io/badge/модули-6-12A387?style=flat-square"/>
  <img src="https://img.shields.io/badge/тарифы-4-2ECC71?style=flat-square"/>
  <img src="https://img.shields.io/badge/SLA-99.95%25-0D7C66?style=flat-square"/>
</p>

---

## Описание

**ООО «ФинДрон»** — IT-агрегатор, специализирующийся на подборе, адаптации и интеграции программных решений для беспилотных авиационных систем (БАС). Компания не является вендором — платформа **ФинДрон HUB** объединяет решения сторонних поставщиков в единую экосистему управления.

Проект разработан в рамках университетской бизнес-игры по моделированию IT-компании в сфере беспилотных систем.

---

## Платформа ФинДрон HUB

Модульная SaaS-платформа управления полным жизненным циклом БАС.

| Модуль | Назначение | Цена/год |
|---|---|---:|
| Управление полётами | Маршруты, координация флота, ФГИС ИВП | 580 000 ₽ |
| Мониторинг и телеметрия | Real-time данные, дашборды, алерты | 520 000 ₽ |
| Нормализация данных | Единый формат, API-шлюз, ETL | 480 000 ₽ |
| Отказоустойчивость | Резервирование каналов, автономные сценарии | 640 000 ₽ |
| Аналитика и отчётность | KPI, прогноз ТО, отчёты для регулятора | 420 000 ₽ |
| Безопасность и комплаенс | Шифрование, IAM, журналирование | 380 000 ₽ |

<p align="center">
  <img src="assets/screenshots/hub_dashboard.png" alt="ФинДрон HUB" width="800"/>
  <br/><em>Центр управления полётами — карта, телеметрия, статус флота</em>
</p>

---

## Техническая архитектура

Архитектура спроектирована для пилотного проекта городской экосистемы беспилотного транспорта в Иннополисе. Подробная документация — [`docs/`](docs/).

```
                    ┌─────────────────────────────────┐
                    │       ФинДрон HUB (ядро)        │
                    │  Агрегационный движок · API-шлюз │
                    └──────────┬──────────────────────┘
                               │
            ┌──────────────────┼──────────────────┐
            │                  │                  │
   ┌────────▼────────┐ ┌──────▼──────┐ ┌─────────▼────────┐
   │  ЦОД            │ │ Телеметрия  │ │ Витрина-         │
   │  Edge + Атомдата│ │ 5G/4G/LoRa  │ │ агрегатор        │
   └─────────────────┘ └─────────────┘ └──────────────────┘
```

**ЦОД:** гибридная модель — 3 Edge-узла (<5 мс) + центральный ЦОД Атомдата (3–8 мс). TCO 3 года: 42–60 млн ₽.

**Телеметрия:** многоканальное резервирование:

| Канал | Технология | Latency | Роль |
|---|---|---|---|
| Основной C2 | 5G SA / LTE Cat-M1 | 10–30 мс | Управление, телеметрия, видео |
| Резервный C2 | 4G LTE (MVNO) | <50 мс | Автопереключение <2 сек |
| IoT | LoRaWAN (868 МГц) | 0.5–2 сек | ПВЗ, зарядки, постаматы |
| Ближняя зона | Wi-Fi 6E / HaLow | 1–5 мс | Доки, склады |
| Навигация | GNSS + RTK | <1 мс | Точность <2 см |
| Аварийный | Спутник LEO VSAT | 20–80 мс | Потеря всех каналов |

**Интеграции:**

| Система | Протокол | Назначение |
|---|---|---|
| Яндекс Go | REST API | Статус дорог ↔ воздушное пространство |
| Светофоры | OPC-UA / MQTT | Приоритетная фаза при посадке |
| ЕДДС-112 | REST + WebHook | Карточка инцидента |
| ФГИС ИВП | REST API | Автоподача планов полётов |
| ЦОД Атомдата | VPN | Вычисления, хранение |
| Цифровой двойник | GraphQL / REST | 3D-карта + позиции дронов RT |

**ИБ:** TLS 1.3, AES-256, DTLS (C2), MFA, SIEM, ФЗ-152.

### План реализации — 30 недель

| Этап | Срок | Задачи | CAPEX |
|---|---|---|---:|
| 0. Подготовка | Нед. 0–2 | Аудит 5G/4G, обследование площадок | — |
| 1. Проектирование | Нед. 2–6 | Архитектура Edge/ЦОД, ТЗ на интеграции | 1–2 млн |
| 2. Инфраструктура | Нед. 6–14 | 3 Edge-узла, LoRaWAN, подключение ЦОД | 6–8 млн |
| 3. Интеграции | Нед. 10–18 | ФГИС ИВП, Яндекс Go, АСУДД, ЕДДС-112 | 3–4 млн |
| 4. Витрина MVP | Нед. 14–22 | Приложение, онбординг 3–5 клиентов | 2–3 млн |
| 5. Пилот | Нед. 22–30 | 20 дронов, ситуационный центр | ~2 млн |
| 6. Масштабирование | После 30 | Новые операторы, география | 10–14 млн/год |

**Итого CAPEX:** 18–28 млн руб. · **OPEX/год:** 10–14 млн руб.

---

## Тарифы

| | Старт | Бизнес | Премиум | Enterprise |
|---|:---:|:---:|:---:|:---:|
| **Цена** | 480 ₽ | 890 ₽ | 1 490 ₽ | 490 000 ₽ |
| **Модель** | борт/мес. | борт/мес. | борт/мес. | фикс./мес. |
| **Бортов** | до 10 | до 100 | до 499 | от 500 |
| **Модули** | 3 | 5 | все 6 | 6 + кастом |
| **SLA** | 99.5% | 99.7% | 99.9% | 99.95% |
| **Поддержка** | 8/5 | 12/7 | 24/7 | 24/7 + TAM |

При оплате за год — скидка 20%. Enterprise: фиксированная цена независимо от количества бортов.

### Дополнительные услуги

| Услуга | Цена | Единица |
|---|---:|---|
| Интеграция с ERP/SCADA | 380 000 ₽ | за систему |
| Обучение персонала | 18 000 ₽ | за чел. |
| Адаптер для БАС | 180 000 ₽ | за тип |
| Пилотный запуск | 320 000 ₽ | фикс. |
| Техподдержка 24/7 | 45 000 ₽ | мес. |
| White-label | 380 000 ₽ | фикс. |

### Примеры расчётов

| Сценарий | Бортов | Тариф | Итого/год | На борт/мес. |
|---|---:|---|---:|---:|
| Малый парк | 10 | Старт | 611 600 ₽ | 5 097 ₽ |
| Средний парк | 50 | Бизнес | 2 171 200 ₽ | 3 619 ₽ |
| Крупный парк | 200 | Премиум | 6 444 720 ₽ | 2 685 ₽ |
| Enterprise | 750 | Enterprise | 11 384 000 ₽ | 1 265 ₽ |

---

## Витрина услуг

Интерактивная витрина с калькулятором стоимости: [`ui/findron_services_storefront.html`](ui/findron_services_storefront.html)

<p align="center">
  <img src="assets/screenshots/storefront_hero.png" alt="Витрина" width="800"/>
  <br/><em>Рис. 1. Главная страница витрины — навигация, ключевые показатели и точка входа</em>
</p>
<p align="center">
  <img src="assets/screenshots/storefront_modules.png" alt="Модули" width="800"/>
  <br/><em>Рис. 2. Каталог модулей платформы ФинДрон HUB — выбор и конфигурация сервисов</em>
</p>
<p align="center">
  <img src="assets/screenshots/storefront_pricing.png" alt="Тарифы" width="800"/>
  <br/><em>Рис. 3. Тарифные планы — Старт, Бизнес, Премиум и Enterprise</em>
</p>
<p align="center">
  <img src="assets/screenshots/storefront_addons.png" alt="Допуслуги" width="800"/>
  <br/><em>Рис. 4. Дополнительные услуги — интеграция, обучение, адаптеры и поддержка</em>
</p>
<p align="center">
  <img src="assets/screenshots/storefront_calc.png" alt="Калькулятор" width="800"/>
  <br/><em>Рис. 5. Интерактивный калькулятор — автоматический расчёт под параметры клиента</em>
</p>

---

## Финансовые показатели

| Показатель (тыс. ₽) | 2025 | 2024 | 2023 |
|---|---:|---:|---:|
| Выручка | 1 570 020 | 1 207 710 | 760 953 |
| Себестоимость | (612 908) | (482 308) | (319 600) |
| Валовая прибыль | 957 112 | 725 402 | 441 353 |
| Чистая прибыль | 506 406 | 373 306 | 211 762 |
| Баланс (активы) | 1 167 000 | 859 858 | 348 201 |
| Собственный капитал | 785 094 | 514 688 | 211 772 |
| Денежные средства | 542 600 | 398 258 | 86 101 |

Рентабельность продаж: 32.3% · Рентабельность активов: 43.4% · Рост выручки YoY: +30% · Текущая ликвидность: 2.77 · Финансовая устойчивость: 0.67

---

## Структура репозитория

```
FIN_Drone/
│
├── assets/
│   ├── logos/                         — логотипы компании и партнёров
│   │   ├── findron.png
│   │   ├── geoscan.png
│   │   ├── supercam.png
│   │   ├── aeromax.png
│   │   └── albatros.png
│   └── screenshots/                   — скриншоты интерфейсов
│       ├── hub_dashboard.png
│       ├── storefront_hero.png
│       ├── storefront_modules.png
│       ├── storefront_pricing.png
│       ├── storefront_addons.png
│       └── storefront_calc.png
│
├── docs/                              — проектная документация
│   ├── Финдрон Корпоративный Профиль.docx
│   ├── Финдрон Корпоративный Профиль.pdf
│   ├── Концепция Игры.docx
│   ├── Глоссарий_Проекта_ИТ-Интегратор.docx
│   └── Роли_Участники.xlsx
│
├── finance/
│   ├── ready/                         — КП к отправке
│   │   ├── Коммерческое_Предложение_WORD.zip
│   │   └── Коммерческое_Предложение_PDF.zip
│   ├── reports/                       — финансовая отчётность
│   │   ├── Отчетности.xlsx
│   │   └── СПАРК_Отчет_ООО_АТИ_СУ_7816397410...
│   └── templates/                     — шаблоны КП и КО
│       ├── Финдрон_КП_Шаблон.docx
│       └── Финдрон_КО_Шаблон.docx
│
├── ui/                                — интерфейсы (HTML)
│   ├── findron_hub.html
│   └── findron_services_storefront.html
│
└── README.md
```

---

## Коммерческие предложения

В архивах `finance/ready/` содержатся 14 персонализированных КП:

<details>
<summary>Развернуть список</summary>

| # | Тип организации | Бюджет проекта |
|---|---|---:|
| 1 | Проектная организация | 3 320 000 ₽ |
| 2 | Дорожно-строительная организация | 4 210 000 ₽ |
| 3 | Электросетевая организация | 5 990 000 ₽ |
| 4 | Оператор мобильной связи | 4 840 000 ₽ |
| 5 | Провайдер кабельных сетей | 3 560 000 ₽ |
| 6 | Банк-экосистема | 7 090 000 ₽ |
| 7 | Агрегатор транспортных услуг | 6 860 000 ₽ |
| 8 | Производитель БАС | 4 640 000 ₽ |
| 9 | Сервисная компания БАС | 4 750 000 ₽ |
| 10 | Эксплуатант БТС | 7 030 000 ₽ |
| 11 | Сеть ресторанов | 4 940 000 ₽ |
| 12 | Продуктовый супермаркет | 4 080 000 ₽ |
| 13 | Электронный маркетплейс | 7 400 000 ₽ |
| 14 | Курьерская служба | 6 060 000 ₽ |

</details>

---

## Технологический стек

| Слой | Технологии |
|---|---|
| Backend | Kubernetes, gRPC, REST API |
| Телеметрия | MQTT, MAVLink, DJI SDK, WebSocket |
| Хранение | PostgreSQL, TimescaleDB, S3 |
| Edge | NVIDIA Jetson AGX Orin, FPGA |
| Связь | 5G SA, 4G, LoRaWAN, Wi-Fi 6E, UWB, LEO VSAT |
| Безопасность | TLS 1.3, AES-256, DTLS, MFA, SIEM |
| Frontend | React, WebSocket |
| Мониторинг | Prometheus, Grafana |

---

## Партнёры

<p align="center">
  <img src="assets/logos/geoscan.png" alt="Геоскан" height="55"/>
  &nbsp;&nbsp;&nbsp;&nbsp;
  <img src="assets/logos/supercam.png" alt="Supercam" height="55"/>
  &nbsp;&nbsp;&nbsp;&nbsp;
  <img src="assets/logos/aeromax.png" alt="Аэромакс" height="55"/>
  &nbsp;&nbsp;&nbsp;&nbsp;
  <img src="assets/logos/albatros.png" alt="Альбатрос" height="55"/>
</p>
